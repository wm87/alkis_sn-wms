...
import() {
	...
	*)
	...
	echo "RUNNING: /opt/gdal/bin/ogr2ogr -f $DRIVER $opt $sf_opt -update -append \"$DST\" $CRS \"$dst1\"" | sed -Ee 's/password=\S+/password=*removed*/'
	eval /opt/gdal/bin/ogr2ogr -f $DRIVER $opt $sf_opt -update -append \"$DST\" $CRS \"$dst1\"
	local r=$?
	t1=$(bdate +%s)
...
GDAL_VERSION=$(unset CPL_DEBUG; /opt/gdal/bin/ogr2ogr --version)
echo $GDAL_VERSION
